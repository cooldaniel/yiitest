...
(function($){
    ...
    $.fn.wBox = function(options){
        ...
        function handleClick(){
			...
			if (_this.YQ.requestType && $.inArray(_this.YQ.requestType, ['iframe', 'ajax','img'])!=-1) {
				...
				if (_this.YQ.requestType === "img") {
					...
					if(_this.YQ.imgWidth)
					{
						img.width(_this.YQ.imgWidth);
					}
					img.load(function(){
					...
					if (_this.YQ.requestType === "ajax") {
						...
						ifr.load(function(){
							try {
								...
								是否自适应窗口
								+++++*/
								_fitForWin	=	_this.YQ._fitForWin;
								newW = _fitForWin?Math.min(w.width() - 40, fW):fW;
								newH = w.height() - 25 - (_this.YQ.noTitle ? 0 : 30);
								newH = _fitForWin?Math.min(newH, fH):fH;
								if (!newH) 
									return;
        ...
        function calPosition(w){
            l = ($win.width() - w) / 2;
			yPos	=	_this.YQ.yPos;
			yPos	=	yPos	==	null?9:yPos
            t = $win.scrollTop() + $win.height() /yPos;
            return [l, t];
        }
        /*
         * 拖拽函数drag
        ...
        this.close=function (){
            if (C) {
                ...
                C.stop().fadeOut(300, function(){
                    C.remove();
					typeof _this.YQ.closeBack === 'function' ? _this.YQ.closeBack() : null;
                })
            }
        }
        /*
